...
class TestBuildQrContext:
    """Unit tests for _build_qr_context function."""

    def test_build_qr_context_with_all_fields(self) -> None:
        """Verify context builds correctly with all fields populated.

        Real-world significance:
        - Complete client record from preprocessing
        - All placeholders available for template formatting
        """
    ...
    def test_build_qr_context_french_language(self) -> None:
        """Verify context for French-language client.

        Real-world significance:
        - Quebec and other Francophone deployments
        - language_code must be 'fr' for French notices
        """
        context = generate_qr_codes._build_qr_context(
            client_id="12345",
            first_name="Jean",
    ...
    def test_build_qr_context_handles_none_values(self) -> None:
        """Verify None/empty values convert to empty strings.

        Real-world significance:
        - Incomplete client records shouldn't crash formatting
        - Missing fields should produce empty string placeholders
        """
        context = generate_qr_codes._build_qr_context(
            client_id="12345",
            first_name="",
    ...
    def test_build_qr_context_combines_name(self) -> None:
        """Verify 'name' field combines first and last name.

        Real-world significance:
        - Some QR templates use {name} instead of separate first/last
        - Must handle partial names (missing first or last)
        """
        # Both names
        context = generate_qr_codes._build_qr_context(
            client_id="1",
    ...
    def test_build_qr_context_strips_whitespace(self) -> None:
        """Verify whitespace is stripped from field values.

        Real-world significance:
        - Input data may have leading/trailing spaces from extraction
        - Must normalize for clean QR payloads
        """
        context = generate_qr_codes._build_qr_context(
            client_id="  12345  ",
            first_name="  John  ",
